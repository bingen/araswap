...
contract Araswap is AragonApp {
    ...
    function _ethToTokenInput(uint256 eth_sold, uint256 min_tokens, uint256 deadline, address buyer, address recipient) private returns (uint256) {
        ...
        uint256 token_reserve = token.balanceOf(address(this));
        uint256 tokens_bought = _getInputPrice(eth_sold, address(this).balance.sub(eth_sold), token_reserve);
    ...
    function _tokenToEthInput(uint256 tokens_sold, uint256 min_eth, uint256 deadline, address buyer, address /* payable */ recipient) private returns (uint256) {
